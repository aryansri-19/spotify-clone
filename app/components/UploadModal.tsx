...
const UploadModal = () => {
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (values) => {
        try{
            ...
            {
                setIsLoading(false);
                return toast.error("Falied to upload song")
            }
        ...
        <Modal title="Add a song" description="Upload an mp3 file" isOpen={uploadModal.isOpen} onChange={onChange}> 
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-y-4">
                ...
                <div>
                    ...
                    <Input type='file' id='song' disabled={isLoading} {...register('song', { required: true })} accept='mp3'/>
                </div>
                <div>
                    <div className="pb-1">
                        Select an image
                    </div>
                    <Input type='file' id='image' disabled={isLoading} {...register('image', { required: true })} accept='image/*'/>
                </div>
